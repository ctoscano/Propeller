 ...
 *  
 *  Read More -- https://github.com/kriszyp/multi-node#readme
 *  
 *  @author kriszyp
 *  @see https://github.com/kriszyp/multi-node
 */
			...
			process.addListener(signal, function(){
				...
				if(signal !== 'exit' && signal !== 'SIGHUP'){
				    if (children.length == 0) {
				        console.log('all child processes have ended; exiting main process');
				        process.exit();
				    } else {
				        console.log('waiting for ' + children.length + ' child processes to end');
				        setTimeout(process.kill, 100);
				    }
				}
			});
		});
